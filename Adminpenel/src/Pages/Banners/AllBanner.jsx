...
const AllBanner = () => {
    ...
    const deleteBanner = async (_id) => {
        setLoading(true);
        try {
            const res = await axios.delete("http://localhost:8080/api/banner/" + _id);
            if (res.status === 200) {
                    ...
                    <tbody>
                        {currentData.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td><img src={item.bannerImage} alt="" style={{ height: 100, width: "auto" }} /></td>
                                <td><Link className="bt edit" to={`/edit-banner/${item._id}`}>Edit <i className="fa-solid fa-pen-to-square"></i></Link></td>
                                <td><button className="bt delete border-0" onClick={() => deleteBanner(item._id)}>{loading ? "Wait..." : "Delete"} <i className="fa-solid fa-trash"></i></button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
